...
We can thus invoke \magicref{SpecialInitialIntrosp} to obtain a unique introspective theory $\langle T, f[C], \introS, \introN \rangle$ such that $\introS$ is a map in $\initMod{Th}$.
\end{construction}
...
Note that this structure contains true arithmetic (in the sense of all true statements in the language of first-order arithmetic; note that determining membership in this set of statements is highly uncomputable), and has uncountable hom-sets (thus, too large to admit \Goedel/ coding by mere natural numbers), but like any geminal category is still subject to \Loeb/'s theorem and \Goedel/'s incompleteness theorems. It's just that its consistency sentence isn't expressible in first-order arithmetic; that is, this structure is not definable in first-order arithmetic. Thus, we see quite vividly that these \Goedel/-\Loeb/ phenomena are not constrained to having anything to do with computability or even countability.
...
Furthermore, suppose given some subset of the morphisms of $S$ which is closed under composition on either side with arbitrary morphisms. That is, suppose given some bifunctor $\SpecialHom_S(-, =) : S^{op} \times S \to \Set$ along with an inclusion map from $\SpecialHom$ to $\Hom_S(-, =) : S^{op} \times S \to \Set$.\footnote{This construction would work just as well for any bifunctor $\SpecialHom$ with a map to $\Hom$. It is not actually necessary that this map be monic. We use the language of \quote{subfunctor} and \quote{inclusion} here just for linguistic convenience, and because our archetypal examples happen to be of this form.}
