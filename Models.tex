...
Let $T$ be a lexcategory (whose models we shall call \quote{gadgets}), let $C$ be a lexcategory internal to $T$ (such that we think of the interpretation of $C$ as the \quote{underlying lexcategory} of any gadget), and suppose there is an initial model of $T$ in $\Glob{C}$.

In other words, the initial gadget $G$ (in $\Set$) contains, in its underlying lexcategory, an initial internal gadget $G'$.

For sake of a name for this concept, call such a theory $T$ \defined{\initogeminal/}.

\TODOinline{See note at \magicref{ModelTerminology} on the inaptness of this "internal" terminology. Clean it up.}

(With most \initogeminal/ theories of interest, it is furthermore the case that for any lexfunctor $f : T \to S$ for arbitrary $S$, we have that $\InducedHomo{f}{G'}$ is an initial model of $T$ in $\Glob{f[C]}$; in other words, EVERY model of the theory has an internal initial model of the theory, and every homomorphism preserves this internal initial model. That is, with most \initogeminal/ theories of interest, the initial model's internal initial model is obtained not just accidentally because of some paucity of other globally defined structures, but intrinsically by virtue of the algebraic laws encoded in the theory. But we do not actually need this property for the following construction.)
...
\end{theorem}
\begin{proof}
By \magicref{CommaKan}, with $X$ and $Y$ as $S$, with $\omega$ as $U$, with $f_X$ as $!$ and $f_Y$ as an arbitrary lexfunctor, with $A$ as the initial lexcategory and $B$ as $T$, we get that $(!/f_Y)$ has an initial model of $T$, created by both projections out of this comma category. There is therefore a unique lexfunctor from $S$ to $(!/f_Y)$ preserving initial models of $T$, and this condition is on such a lexfunctor is the same as asking for both its projections to $S$ to preserve initial models of $T$, which in turn is the same as asking for both projections to be the identity on $S$. Thus, there is a unique natural transformation from $! \circ id_S$ to $f_Y \circ \id_S$.
\end{proof}

Therefore, the identity endolexfunctor on the initial object $S$ of $\initMod{T}$ is initial among its endolexfunctors. \TODOinline{In the language of the following todo note, this is sigma1esque}. In this way, we get an introspective theory on this $S$ once it also has a lexfunctor in the globalization of some internal category in $T$ (or, for that matter, in itself...).

\subsubsection{Stray TODO notes mostly already taken care of}
...
Every arithmetic functor out of the initial arithmetic universe $\IAU$ is an initial model of $\IAU$. That is, if $C$ is any other arithmetic universe, $!_C : \IAU \to C$ is the unique arithmetic functor between these, and $f: \IAU \to C$ is an arbitrary lexfunctor, then there is a unique natural transformation from $!_C$ to $f$.
\end{theorem}
\begin{proof}
From \magicref{CommaStableArithmetic}, we know that the comma category $(\id_C / f)$ is an arithmetic universe, such that its two projections jointly create arithmetic universe structure (that is, a functor into this comma category is arithmetic just in case its compositions with both projections are arithmetic).
...
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	\IAU \\
	& {(\id_C / f)} && C \\
	\\
