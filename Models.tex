...
Finally, we can define the sets of arbitrary or well-founded trees similarly, as suitable partial functions with domain $\nat^\nat$ and codomain the set of labels (returning the label found by traversing a given sequence of branches down from the root). We can express within the internal logic of topos theory the conditions corresponding to be a tree, as well as the further condition corresponding to being well-founded (by a suitable quantification over the power object), as well as the further conditions corresponding to representing a well-formed term or equation in our algebraic theory. We can thus take the appropriate subobject of the set of all such partial functions, to get the set of well-formed well-founded labelled trees we are interested in.
