...
Furthermore, suppose given an initial gadget $G$ with an initial internal gadget $G'$. That is, suppose given some lexcategory $S$ such that $\LexCat(T, S)$ has an initial object (our $G$) and such that $\LexCat(T, G[C])$ has an initial object (our $G'$).

Because $G$ is initial, we automatically get a unique homomorphism $\introN_{G}$ from $G$ to $\Hom_{|G|}(1, G')$. And because $G'$ is an initial $|G|$-internal gadget, we automatically get a unique $|G|$-internal homomorphism $\introN_{G'}$ from $G'$ to $\Hom_{|G'|}(1, G'')$ where $G'' = \introN_{G}[G']$.

This setup is thus a geminal gadget (with the equations $E_1$ and $E_2$ of \magicref{CompactGeminalCatDefn} automatically satisfied by the uniqueness observations in the previous paragraph).

Indeed, this is the unique way to equip $\langle G, G' \rangle$ as a geminal gadget.
\end{construction}

In practice, when an initial gadget has an initial internal gadget like above, this is usually not just some accident caused by a paucity of globally defined structures, but rather, is due to the theory of gadgets itself encoding the construction of an internal initial gadget:
\begin{definition}
...
If every gadget has an initial internal gadget in its underlying lexcategory, and every gadget homomorphism preserves these initial internal gadgets, then we say the theory of gadgets is \defined{self-initializing}.

In other words, $T$ is self-initializing if $\LexCat(T, \Glob{C})$ has an initial object, and this initiality is preserved by every $\InducedHomo{f}{C}$ for any lexfunctor $f$ out of $T$.
\end{definition}

There are a number of self-initializing theories in the wild, which thus immediately give us examples of geminal categories in the wild.

For example: \TODO.

The above all admits a slight generalization worth noting:

\begin{construction}\label{InitoGeminalYieldsGeminal}
Suppose given some lexfunctor $i : T \to T_2$, along with a lexcategory $C$ internal to $T$. We call $T$ the theory of \quote{gadgets}, and $T_2$ the theory of \quote{supergadgets}. Via $i$, every supergadget has an underlying gadget, and via $C$, every gadget has an underlying lexcategory.

Furthermore, suppose given an initial gadget $G$ with an initial internal supergadget $G'$. That is, suppose given some lexcategory $S$ such that $\LexCat(T, S)$ has an initial object (our $G$) and such that $\LexCat(T_2, G[C])$ has an initial object (our $G'$).

Because $G$ is initial, we automatically get a unique gadget homomorphism $\introN_{G}$ from $G$ to $\Hom_{|G|}(1, G')$. And because $G'$ is an initial $|G|$-internal supergadget, we automatically get a unique $|G|$-internal supergadget homomorphism $\introN_{G'}$ from $G'$ to $\Hom_{|G'|}(1, G'')$ where $G'' = \introN_{G}[G']$.

This setup is thus a geminal gadget (with the equations $E_1$ and $E_2$ of \magicref{CompactGeminalCatDefn} automatically satisfied by the uniqueness observations in the previous paragraph).

Indeed, this is the unique way to equip $\langle G, G' \rangle$ as a geminal gadget $\langle G, G', F, F' \rangle$ such that $F'$ comes from a supergadget homomorphism.
\end{construction}
...
\begin{definition}
Suppose, as above, given some lexfunctor $i : T \to T_2$, along with a lexcategory $C$ internal to $T$. We call $T$ the theory of \quote{gadgets}, and $T_2$ the theory of \quote{supergadgets}. Via $i$, every supergadget has an underlying gadget, and via $C$, every gadget has an underlying lexcategory.

If every gadget has an initial internal supergadget in its underlying lexcategory, and every gadget homomorphism preserves these initial internal supergadgets, then we say the theory of gadgets (or more precisely, the extension of the theory of gadgets by the theory of supergadgets) is \defined{super-initializing}.

In other words, this situation is super-initializing if $\LexCat(T_2, \Glob{C})$ has an initial object, and this initiality is preserved by every $\InducedHomo{f}{C}$ for any lexfunctor $f$ out of $T$.
\end{definition}

The self-initializing situation is of course the special case of the super-initializing situation where $T_2 = T$ and $i$ is the identity.

Again, there are a number of self-initializing theories in the wild, which thus immediately give us examples of geminal categories in the wild.

For example: \TODO.

\TODOinline{Note that when the theory is in fact strongly self-initializing, in the sense of the previous TODO note, then we have that in the geminal category $\langle G_1 = G, G_2 = G', \ldots; F, \ldots \rangle$, $G_3 = G''$ is also an initial gadget internal to $G'$ (since $G''$ is given by $F[G']$). Thus, $\InteriorGeminal{G}$ is itself given by the internalization of the same construction of a geminal category for a self-initializing theory.}

\TODOinline{Discuss more generally how we can get geminal categories from the initial X, if the theory X is extended by the theory Y, and the initial X has an internal initial Y. The above is the special case of this where X and Y coincide.}
...
\TODOinline{In addition to our example of an \initogeminal/ theory with uncountable flavor, add one with uncomputable flavor.}

\subsubsection{The theory of initial models of a \initogeminal/ theory as an introspective theory}
\TODOinline{Come up with a less wordy section title}
...
More generally, let $T$ and $T_2$ be lexcategories, and $Extend : T \to T_2$ be given, such that $T_2$ is thought of as a lex theory extending $T$. \TODO
