...
\subsection{Self-initializing theories}

\subsubsection{The initial model as a geminal category}
\begin{construction}\label{InitoGeminalYieldsGeminal}
Let $T$ be a lexcategory (whose models we shall call \quote{gadgets}), let $C$ be a lexcategory internal to $T$ (such that we think of the interpretation of $C$ as the underlying lexcategory of any gadget), and suppose there is an initial model of $T$ in $\Glob{C}$.

In other words, the initial gadget $G$ (in $\Set$) contains, in its underlying lexcategory, an initial internal gadget $G'$.

For sake of a name for this concept, call such a theory $T$ \defined{weakly \initogeminal/}.

\TODOinline{See note at \magicref{ModelTerminology} on the inaptness of this "internal" terminology. Clean it up.}

(With most weakly \initogeminal/ theories of interest, it is furthermore the case that for any lexfunctor $f : T \to S$ for arbitrary $S$, we have that $\InducedHomo{f}{G'}$ is an initial model of $T$ in $\Glob{f[C]}$; in other words, EVERY model of the theory has an internal initial model of the theory, and every homomorphism preserves this internal initial model. That is, with most weakly \initogeminal/ theories of interest, the initial model's internal initial model is obtained not just accidentally because of some paucity of other globally defined structures, but intrinsically by virtue of the algebraic laws encoded in the theory. In that case, we shall call the theory \defined{strongly \initogeminal/}. But we do not actually need this property for the following construction.)
...
In summary, we have shown how the initial model of any weakly \initogeminal/ theory can naturally be equipped as a geminal model of that same theory.
\end{construction}
