...
Let $\IAU$ be the initial arithmetic universe, and let $C$ be any arithmetic universe internal to $\IAU$. Then by the combination of \magicref{SpecialInitialIntrosp} and \magicref{CommaStableArithmetic}, we obtain an introspective theory $\langle T, C, \introS, \introN \rangle$ in which $\introS : T \to \Glob{C}$ is the unique such arithmetic functor, and the natural transformation $\introN : \id_T \to \Hom_C(1, \introS(-))$ is uniquely determined.
\end{construction}
...
We now use \magicref{IAUAsIntrospGeneral} to give a fuller account of our original guiding examples of introspective theories based on traditional logical theories, from \magicref{SigmaModelSimple} and \magicref{SigmaModelComplex}:

\begin{construction}\label{MaxLocalizeIAUGeneral}
Given any arithmetic universe internal to the initial arithmetic universe $\IAU$, we may consider the introspective theory obtained by first using the construction \magicref{IAUAsIntrospGeneral} and then taking its maximal localization in the sense of \magicref{LocalizeIntrosp}.
\end{construction}

\begin{theorem}
The introspective theory described in \magicref{SigmaModelSimple} is the result of applying \magicref{MaxLocalizeIAUGeneral} to the lexcategory $\ZFin_{\Sigma_1}$ internal to IAU.

Similarly, the introspective theory described in \magicref{SigmaModelComplex} is the result of applying \magicref{MaxLocalizeIAUGeneral} to the lexcategory $\ZFin$ internal to IAU.
\end{theorem}
\begin{proof}
\TODO
\end{proof}
