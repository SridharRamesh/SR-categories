...
Thus, we will have that $C$ is $\KPshGlobal{K_S}{S}$-\repsmall/, yielding an introspective theory, just in case for each $s \in S$, we have that $K_S(s)$ contains a certain set (the $s$-aspect of $\Mor(C)$) determined by the values of $K_S(t)$ at $t < s$. If the $<$ relation is well-founded, we can readily recursively choose the values of $K_S$ to always satisfy this condition, by simply choosing at each stage the minimal full sublexcategory of $\Set$ generated by the required set. (Indeed, we could just as well arrange in essentially the same way for each $K_S(s)$ to also any \setsmall/ other number of sets of interest.)

Thus, when the $<$ relation corresponding to $\SpecialHom_S$ is well-founded, we can choose a ramp $K_S$ such that we obtain an introspective theory by the above contruction (and we can furthermore do so in such a way that $\KPshGlobal{K_S}{S}$ includes many other desired objects of $\Psh{S}$, so long as the number of desired objects is \setsmall/).
