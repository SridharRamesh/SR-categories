...
\TODOinline{Rewrite this just in terms of the internal $\Box_C$ in the initial introspective theory, which then automatically applies to arbitrary geminal categories. Also discuss how \Loeb/'s theorem holds in any geminal category.}
...
\subsection{Co-free introspective theories and geminal categories}
...
\end{construction}
\openDetails
For linguistic convenience, we discuss the construction with $V$ taken as $\Set$, but it will be manifest that the same explicit construction works for any ambient lexcategory $V$.
