...
Furthermore, we have a projection lexfunctor $\pi : Z \to C_1$, sending coalgebras to their carriers. By construction, this is a strict lexfunctor which takes $Z_2$ to $C'_2$ and takes $\introN_{Z_2}$ to $F'_2$, while furthermore the following diagram commutes:
...
The condition $\pi \circ \beta = H$ tells us right away what the carriers of the coalgebras and coalgebra morphisms produced by $\beta$ must be. Furthermore, our construction of $\introN_z$ for objects $z$ of $Z$ is such $\pi (\introN_z)$ in $C'_1$ is the very same as the coalgebra $z$ itself. Thus, for any object $t \in T$, the specific coalgebra $\beta(t)$ must be the one given by $\pi (\introN_{\beta(t)})$, which by virtue of $\beta$ being a strict introspective theory homomorphism is the same as $\pi (\beta (\introN_t)) = H(\introN_t)$.
