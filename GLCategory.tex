...
\begin{observation}\label{GlobOfGlobUndetailed}
If $C$ is a lexcategory, $D$ is a $C$-indexed locally \repsmall/ lexcategory, and $S$ is a $\Glob{D}$-internal structure, then the global aspect of $S$ is the same as the global aspect of $\Gamma_D(S)$. \sTODOinline{Write out why this is so}
\end{observation}
...
\newcommand{\GlobIso}{\alpha}
\begin{observation}\label{GlobOfGlob}
Expanding on \magicref{GlobOfGlobUndetailed}, if $C$ is a lexcategory, $D$ is a $C$-indexed locally \repsmall/ lexcategory, and $S$ is a $\Glob{D}$-internal structure, then the map $\InducedHomo{\Gamma_D}{S} : \Glob{S} \to \Glob{\Gamma_D[S]}$ is an isomorphism.\sTODOinline{Write out why this is so} We refer to this isomorphism as $\GlobIso_S$.
\end{observation}
