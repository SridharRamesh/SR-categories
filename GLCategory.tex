...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{C_i = \Glob{C'_i}} && {\Glob{\Gamma_i[C_{i + 1}]}} \\
	\\
	&& {C_{i + 1} = \Glob{C'_{i + 1}}}
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{C'_i} &&& {\Gamma_i [C'_{i + 1}]} \\
	\\
	{\Gamma_i [C'_{i + 1}]} && {\Gamma_{\Gamma_{i}[C'_{i + 1}]} [F'_i[C'_{i + 1}]]} & {\Gamma_i [\Gamma_{i + 1} [C'_{i + 2}]]}
...
That is, we require that $\InducedHomo{F'_i}{C'_{i + 1}} \circ F'_i = \Gamma_i[F'_{i + 1}] \circ F'_i$. This could be glossed in abuse of notation as \quote{$F'_i \circ F'_i = F'_{i + 1} \circ F'_i$}, as it amounts to saying in the internal logic of $C_{i - 1}$ that $F'_i [F'_i[x]] = F'_{i + 1}[F'_{i}[x]]$ for all structures $x$ internal to $C'_i$.\footnote{Keeping in mind that in $F'_i [F'_i[x]]$ the outer $F'_i$ is interpreted as $\InducedHomo{F'_i, C'_{i + 1}}$ a la \magicref{TransferNDefn}, while in $F'_{i + 1}[F'_{i}[x]]$ the application of the internal lexfunctor $F'_{i + 1}$ to a value in the global aspect $\Gamma_i [C'_{i + 1}]$ of its domain really means applying the global aspect lexfunctor $\Gamma_i[F'_{i + 1}]$.}

(To derive the identity in the bottom-right of the above diagram, first note that $F'_i[C'_{i + 1}] = \InducedHomo{\Gamma_i}{C'_{i + 1}} [ \straight{F}_i[C'_{i + 1}]] = \InducedHomo{\Gamma_i}{C'_{i + 1}} [C'_{i + 2}]$. 
...
\end{theorem}
\begin{proof}
Throughout the following, as before, we define each $\straight{F}_i$ from the corresponding $F_i$ by $\InducedHomo{\Gamma_i}{C'_{i + 1}} \circ \straight{F}_i = F_i$.

By definition, in a geminal category, we must have that $\straight{F}_1[C'_j] = C'_{j + 1}$ and $\straight{F}_1[F'_j] = F'_{j + 1}$ for each $j > 2$.

Accordingly, if we are given the structure in \magicref{CompactGeminalCatDefn}, and we are to extend it to all the further structure in \magicref{VerboseGeminalCatDefn}, it inductively must be the case that $C'_j = \straight{F}_1^{j - 2}[C'_2]$ and $F'_j = \straight{F}_1^{j - 2}[F'_2]$ for each $j > 2$. Adopt these definitions throughout the following accordingly.
...
What remains is only to see that each $\straight{F}_i$ for $i > 1$ also takes $C'_j$ to $C'_{j + 1}$ and takes $F'_j$ to $F'_{j + 1}$, for $j > i \geq 1$.

We prove this by induction on $i$. For the base case of $i = 1$, we have ensured this by construction. As for the inductive step, suppose we know this already holds for $i$. Then $\straight{F}_{i + 1}[C'_j] = \straight{F}_{i + 1} [\straight{F}_i [C'_{j - 1}]] = \straight{F}_i [\straight{F}_i [C'_{j - 1}]] = \straight{F}_i [C'_j] = C'_{j + 1}$, where the second step is by the global aspect of $E_i$ and the other steps are by our induction hypothesis. And similarly with $F'$ in place of $C'$ throughout as well.
\end{proof}
