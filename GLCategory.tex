...
This differs from the definition of an introspective theory in precisely the manner that $T$ is made strict (thus, its internal structures can be considered up to equality instead of mere isomorphism), we demand the selection of $C$ as a particular $T$-internal lexcategory up to equality (instead of simply up to presenting equivalent indexed categories), and we demand that $\introS$ preserves strict structure (that is, preserves chosen basis limits).
...
Now using \TODO, let $T_{strict}$ be some strict lexcategory which presents $T$ and which has the freeness property that \TODO. Because $T'$ presents $T$, we can choose some specific internal lexcategory $C_{strict}$ in $T_{strict}$ (up to equality!) which presents $C_{int}$. Furthermore, using the freeness property of $T_{strict}$, we obtain a lexfunctor $\introS_{strict}$ from $T_{strict}$ to the global aspect of $C_{strict}$ which presents $\introS$. Finally, we deal with $\introN$. \TODO
\end{proof}
...
% https://q.uiver.app/?q=WzAsNCxbMCwwLCJUXzEiXSxbMSwwLCJUXzIiXSxbMCwxLCJcXEdsb2J7Q18xfSJdLFsxLDEsIlxcR2xvYntDXzJ9Il0sWzAsMSwiSCJdLFswLDIsIlxcaW50cm9TXzEiLDJdLFsyLDMsIkhbLV0iLDJdLFsxLDMsIlxcaW50cm9TXzIiXV0=
\[\begin{tikzcd}
	{T_1} & {T_2} \\
	{\Glob{C_1}} & {\Glob{C_2}}
	...
	\arrow["{\introS_2}", from=1-2, to=2-2]
\end{tikzcd}\]
...
Recall from \magicref{MultiplyInternal} that if $F : C \to D$ is a lexfunctor, and $S$ is some structure internal or $n$-tuply internal to $C$, then $F[S]$ is a similar structure internal or $n$-tuply internal to $D$, and furthermore, $F$ induces a homomorphism from the global aspect of $S$ to the global aspect of $F[S]$, which we may call $\InducedHomo{F}{S}$. \TODOinline{Some random notation choice here, to be revised}

Throughout the following, whenever we write $F[S]$, this means the application of the lexfunctor $F$ to some internal or multiply internal structure $S$. When we write $F \circ G$, this means the composite action of first applying $G$, then applying $F$, to some internal or multiply internal structure (or global aspects thereof). Note, in particular, that both $F[G]$ and $F \circ G$ may be meaningful, in certain contexts where $G$ is an internal or multiply internal lexfunctor itself, but that they mean slightly distinct (though related!) things.
...
\begin{itemize}
    \item 
    The first ingredient is an infinite sequence $C_1, C_2, C_3, \ldots$, in which each $C_{i + 1}$ (for $i \geq 0$) is the global aspect of a lexcategory internal to $C_i$ (let us refer to this internal lexcategory as $C'_{i + 1}$).
\end{itemize}

(Throughout the following, it will be useful to keep in mind that we are using these general naming habits: Primed names are used for internal structures, while the corresponding unprimed names indicate the corresponding global aspects. Furthermore, names subscripted with index $j$ arise from structure internal to $C_{j - 1}$.)

Note that $\Hom_{C'_{i + 1}}(1, -)$ can be seen as a lexfunctor from $C_{i + 1}$ to $C_i$ (since $C'_{i + 1}$ is indexed over $C_i$, this functor takes the global aspect of $C'_{i + 1}$ to the global aspect of $C_i/-$). Call this global sections functor $\Gamma_{i + 1} : C_{i + 1} \to C_i$. In general, it will do no harm to drop the subscript and simply say $\Gamma$.

Observe that, if $S$ is some structure internal to $C_{i + 1}$, then the global aspect of $S$ is the same as the global aspect of the $C_i$-internal structure $\Gamma[S]$. In particular, the global aspect of $\Gamma[C'_i]$ is $C_i$ for each $i \geq 2$.
...
\begin{itemize}
    ...
    \item
    Furthermore, we require an equation internal to $C_{i - 1}$ which informally can be glossed as \quote{$F'_i \circ F'_i = F'_{i + 1} \circ F'_i$}, for each $i \geq 1$. We refer to this equation as $E_i$. More formally, $E_i$ states that the following diagram of lexfunctors in $C_{i - 1}$ commutes:
    
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	{C'_i} && {\Gamma_i [C'_{i + 1}] = \Hom_{C'_i}(1, C'_{i + 1})} \\
	&& {\Hom_{\Gamma_i[C'_{i + 1}]}(1, F'_i[C'_{i + 1}]) = \Hom_{C'_i}(1, \Hom_{C'_{i + 1}}(1, F'_i[C'_{i + 1}]))} \\
	{\Gamma_i [C'_{i + 1}]} && {\Gamma_i [\Gamma_{i + 1}[C'_{i + 2}]] = \Hom_{C'_i}(1, \Hom_{C'_{i + 1}}(1, C'_{i + 2}))}
	\arrow["{F'_i}", from=1-1, to=1-3]
	\arrow["{F'_i}"', from=1-1, to=3-1]
	\arrow["{\Gamma_i [F'_{i + 1}]}"', from=3-1, to=3-3]
	\arrow["{F'_i}", from=1-3, to=2-3]
	\arrow[Rightarrow, no head, from=2-3, to=3-3]
\end{tikzcd}\]
\end{itemize}

The top, left, and bottom of this diagram are straightforward. On the top-right, we have the action of $F'_i$ as a lexfunctor on a particular global section in its domain, taking it to a corresponding global section in its codomain (this is in the internal logic of $C_{i - 1}$ the induced homomorphism $\InducedHomo{F'_i}{C'_{i + 1}}$). And in the bottom-right, we have an equation which follows from our presumption that $F_i[C'_{i + 1}] = C'_{i + 2}$.
...
To be fully explicit, we would describe the above geminal category internal to $C_0$ by enumerating $\langle C'_1, C'_2, C'_3, \ldots; F'_1, F'_2, F'_3, \ldots \rangle$. However, we take the abuse of notation convention of often referencing this geminal category simply via the name of $C'_1$ (the part of the structure which is a lexcategory directly internal to $C_0$).

It is manifestly the case that this definition of a geminal category is essentially algebraic. That is, there is some essentially algebraic theory such that models of that theory internal to $C_0$ are the same thing as geminal categories internal to $C_0$. Of course, geminal categories simpliciter are just models of this theory in the ordinary sense; that is, geminal categories internal to $\Set$. (In this case, $C'_1$ can be identified with its global aspect $C_1$, in the same way that any structure internal to $\Set$ can be identified with its global aspect.).
...
% https://q.uiver.app/?q=WzAsNCxbMCwwLCJDJ18xIl0sWzEsMCwiRCdfMSJdLFsxLDEsIlxcR2xvYntEXzInfSJdLFswLDEsIlxcR2xvYntDXzInfSJdLFswLDEsIkgiXSxbMCwzLCJGJ18xIiwyXSxbMywyLCJcXEluZHVjZWRIb21ve0h9e0MnXzJ9IiwyXSxbMSwyLCJcXHBoaSdfMSJdXQ==
\[\begin{tikzcd}
	{C'_1} & {D'_1} \\
	{\Glob{C_2'}} & {\Glob{D_2'}}
	\arrow["H", from=1-1, to=1-2]
	\arrow["{F'_1}"', from=1-1, to=2-1]
	\arrow["{\InducedHomo{H}{C'_2}}"', from=2-1, to=2-2]
	\arrow["{\phi'_1}", from=1-2, to=2-2]
\end{tikzcd}\]

In this diagram, the bottom arrow is the action of $H$ as a homomorphism from the global aspect of $C'_2$ to the global aspect of $H[C'_2] = D'_2$.
\end{definition}
...
We furthermore have that $F'_1$ acts as a geminal category homomorphism from $K_1$ to $\Gamma_1[K_2]$ (where $\Gamma_1$ is the global sections functor from $C'_1$ to $\Set$).
\end{theorem}
\begin{proof}
This is all direct by definition.

For the first part, each condition imposed upon each $C'_{i}$ or $F'_{i}$ in the definition of a geminal category comes with an analogous condition imposed upon $C'_{i + 1}$ or $F'_{i + 1}$. Thus, it is immediate that $K_2$ is itself a geminal category internal to the global aspect of $C'_1$.

For the second part, the definition of a geminal category directly imposes upon $F_1$ precisely the conditions which are necessary for $F_1$ to comprise a geminal category homomorphism from $K_1$ to $\Gamma_1[K_2]$. \TODO
\end{proof}
...
A \defined{compactly presented geminal category} internal to lexcategory $C_0$ consists of the structure $C'_i$, $F'_i$, and equations $E'_i$ of the verbose presentation, but only for $i \in \{1, 2\}$.

(Here, in interpreting the codomain of $F'_2$, we take $C'_3$ to be $F_1[C'_2]$, and in interpreting the equation $E'_2$, we take $F'_3$ to be $F_1[F'_2]$)
...
\begin{itemize}
    ...
    \item A lexfunctor $F'_1 : C'_1 \to \Gamma_1[C'_2]$, internal to $C_0$. We use $F_1 : C_1 \to C_2$ to denote the global aspect of $F'_1$.
    
    (Here, $\Gamma_1 : C_1 \to C_0$ is the global sections functor $\Hom_{C_1}(1, -)$.)
    
    \item A lexfunctor $F'_2 : C'_2 \to \Gamma_2[C'_3]$, internal to $C_1$.
    
    (Here, $C'_3$ is defined as $F_1[C'_2]$, and $\Gamma_2 : C_2 \to C_1$ is the global sections functor $\Hom_{C_2}(1, -)$.)
    
    \item The equation $F_1 \circ F_1 = F_2 \circ F_1$, internal to $C_0$. We call this equation $E_1$.
    
    \item The equation $F_2 \circ F_2 = F_3 \circ F_2$, internal to $C_1$. We call this equation $E_2$.
...
We prove this by induction on $i$. For the base case of $i = 1$, we have ensured this by construction. As for the inductive step, suppose we know this already holds for $i$. Then $F_{i + 1}[C'_j] = F_{i + 1} [F_i [C'_{j - 1}]] = (F_{i + 1} \circ F_i)[C'_{j - 1}] = (F_i \circ F_i) = F_i [F_i [C'_{j - 1}]] = F_i [C'_j] = C'_{j + 1}$, where the third step is by $E_i$. And similarly with $F'$ in place of $C'$ throughout as well.
\end{proof}
