...
We are tasked with showing that, for any geminal category $C'_1$, there is a suitably terminal introspective theory with a geminal category homomorphism to $C'_1$.

We will first give the details of the construction, and then give the proof that it has the terminality property.

Recall from \magicref{ModalAxiom4Section} that any introspective theory $\langle T, C \rangle$ comes with a $T$-internal endolexfunctor $\Box_C = \introF(\Hom_C(1, -)): C \to C$, along with a $T$-internal natural transformation $4 : \Box_C \to \Box_C \Box_C$ built from $\introN$. As this applies in particular to the introspective theory $\langle \GLCatTheory, |K| \rangle$ of \magicref{GLCatTheoryIsIntrosp} (where $\introF(\Hom_C(1, -)) = $ \TODO), we find that any geminal category $C'_1 = \langle C'_1, C'_2; F'_1, F'_2 \rangle$ \TODOinline{Should make $C_1$ into $C'_1$ throughout} comes with an endolexfunctor $\Box_{C_1} = \Gamma_{C'_2} \circ \straight{F}_1 : C_1 \to C_1$, along with a natural transformation $4 : \Box_{C_1} \to \Box_{C_1} \Box_{C_1}$ corresponding to the action of $F'_2$.
...
Let $Z$ be the full subcategory of those $\Box_{C_1}$-coalgebras with the specified property. It is readily seen that this $Z$ is closed under the finite limits of the category of $\Box_{C_1}$-coalgebras, and thus is itself a strict lexcategory. (Indeed, $Z$ can be defined as the equalizer of two strict lexfunctors from the $\Box_{C_1}$-coalgebras to the $\Box_{C_1}^2$-coalgebras.)
...
This diagram commutes by equation $E_2$. But this is also the commutative diagram which establishes that the internal lexcategory $F'_2: C'_2 \to \Box_{C_1} C'_2$ within the category of $\Box_{C_1}$-coalgebras is furthermore within its subcategory $Z$. When thinking of $F'_2$ as an internal lexcategory within $Z$ in this way, let us call it $Z_2$.

Note that the strict lexfunctor $\straight{F}_1 : C_1 \to \Glob{C'_2}$ is such that for any object or morphism $x$ of $C_1$, if we interpret $\straight{F}_1(x)$ as a morphism from $1$ to $\Ob(C'_2)$ or $\Mor(C'_2)$, we obtain a commutative diagram of the following form:

% https://q.uiver.app/#q=
\[\begin{tikzcd}
	1 &&& {\Ob(C'_2) \; \text{or} \; \Mor(C'_2)} \\
	\\
	{\Box_{C_1} 1 = 1} &&& {\Box_{C_1} \left( \Ob(C'_2) \; \text{or} \; \Mor(C'_2) \right)}
	\arrow["{F'_2}", from=1-4, to=3-4]
	\arrow["{\straight{F}_1(x)}", from=1-1, to=1-4]
	\arrow["{\Box_{C_1} \left( \straight{F}_1(x)  \right) = \Gamma_{C'_2} [\straight{F}_1[\straight{F}_1(x)]]}"', from=3-1, to=3-4]
	\arrow["{!}"', from=1-1, to=3-1]
\end{tikzcd}\]

That this diagram commutes is by equation $E_1$ of $C'_1$ being a geminal category. Thus $\straight{F}_1(x)$ amounts to a global element of $\Ob(Z_2)$ or $\Mor(Z_2)$, and thus $\straight{F}_1$ acts as a strict lexfunctor from $C_1$ to the global aspect of $Z_2$. By composing this with the projection functor from $Z$ to $C_1$, we get a strict lexfunctor $\introS$ from $Z$ to the global aspect of this $Z$-internal lexcategory. (It may be surprising that this $\introS$ will discard all information lost in the projection from $Z$ to $C_1$, but this will indeed be the correct one for our purposes!)

Finally, for $\introN$, we observe by unwinding definitions that $\Hom_{Z_2}(1, \introS(-)) : Z \to Z$ is the functor which takes a coalgebra on carrier object $c$ to the coalgebra $4_c : \Box c \to \Box \Box c$ and which takes morphisms to the corresponding naturality square for $4$. Thus, by the defining condition of $Z$, we get for each $m \in \Ob(Z)$ a map from $m$ to $\Hom_{Z_2}(1, \introS(m))$ whose carrier is the morphism $m$ itself, as described by the following commutative diagram:
...
In the above commutative diagram within $C'_1$, the top arrow is the coalgebra $m$, while the bottom arrow is the coalgebra $\Hom_{Z_2}(1, \introS(m))$. The left arrow is thus the carrier of a coalgebra morphism from the former to the latter, and the diagram overall commutes because this is the defining condition for $m$ to be within $Z$.

These maps from each $m$ to $\Hom_{Z_2}(1, \introS(m))$ comprise a natural transformation $\introN$ between $\id_Z$ and $\Hom_{Z_2}(1, \introS(-))$ whose naturality is demonstrated like so: Consider any two coalgebras $m_1$ and $m_2$ in $Z$ and a coalgebra map $h : m_1 \to m_2$. The condition for $h$ to be a coalgebra map is the very same as the naturality square for this $\introN$:

% https://q.uiver.app/#q=WzAsNCxbMCwwLCJjIl0sWzAsMiwiXFxCb3ggYyJdLFsyLDAsImQiXSxbMiwyLCJcXEJveCBkIl0sWzAsMSwibV8xIiwyXSxbMCwyLCJoIl0sWzIsMywibV8yIl0sWzEsMywiXFxCb3ggaCIsMl1d
\[\begin{tikzcd}
	c && d \\
	\\
	{\Box c} && {\Box d}
	...
	\arrow["h", from=1-1, to=1-3]
	\arrow["{m_2}", from=1-3, to=3-3]
	\arrow["{\Box h}"', from=3-1, to=3-3]
\end{tikzcd}\]

Thus we have an introspective theory $Z = \langle Z, Z_2, \introS, \introN \rangle$.

By construction, the projection functor $\pi : Z \to C_1$, which sends coalgebras to their carriers, is a strict lexfunctor which takes $Z_2$ to $C'_2$ and takes $\introN_{Z_2}$ to $F'_2$, while furthermore the following diagram commutes:

% https://q.uiver.app/#q=WzAsNCxbMCwwLCJaIl0sWzIsMCwiQydfMSJdLFsyLDIsIlxcR2xvYntcXHBpW1pfMl19ID0gXFxHbG9ie0NfMid9Il0sWzAsMiwiXFxHbG9ie1pfMn0iXSxbMCwxLCJcXHBpIl0sWzAsMywiXFxpbnRyb1MiLDJdLFszLDIsIlxcSW5kdWNlZEhvbW97XFxwaX17Wl8yfSIsMl0sWzEsMiwiRidfMSJdXQ==
\[\begin{tikzcd}
	Z && {C'_1} \\
	\\
	{\Glob{Z_2}} && {\Glob{\pi[Z_2]} = \Glob{C_2'}}
...
This establishes $\pi$ as a geminal category homomorphism from $Z$ to $C_1$.

\closeDetails

Having described the introspective theory $Z$ and its geminal category homomorphism $\pi$ to $C_1$, we now prove its terminality among all introspective theories with a designated geminal category homomorphism to $C_1$:
...
This natural transformation from $H$ to $\Box_{C_1} \circ H$ acts as a functor $\beta$ from $T$ to the category of $\Box_{C_1}$-coalgebras, such that $\beta$ followed by the forgetful functor to $C_1$ yields $H$. As $H$ is a strict lexfunctor and this forgetful functor creates basic limits (a la \magicref{BoxCoalgebrasInGeminal}), this $\beta$ is also a strict lexfunctor.
...
The required condition of $\beta[C]$ is that this should equal $Z_2$. Note that $\beta(C)$ is the lexcategory in $Z$ corresponding to the internal lexfunctor $H(\introN_C)$ in $C'_1$. By virtue of $H : T \to C'_1$ being a geminal category homomorphism, this $H(\introN_C)$ is $F'_2$, which is indeed our definition of the lexcategory $Z_2$ in $Z$, as required.
...
For this diagram to commute, it suffices to know that following both paths with the projection from $\Glob{Z_2}$ to $\Glob{C'_2}$ commutes. By unwinding definitions, $\beta; \introS$ followed by this projection is $\straight{F'_1}(H(-))$ while $\introS ; \InducedHomo{\beta}{C}$ followed by this projection is $H(\introS(-))$. As $H$ is a geminal category homomorphism from $T$ to $C'_1$, these are indeed the same.

Finally, the required condition on $\beta$ with respect to $\introN$ is that $\beta(\introN_t) = \introN_{\beta(t)}$ for each $t \in T$. Note that $\beta(\introN_t)$ is the coalgebra morphism in $Z$ given by applying $H$ to the naturality square in $C'_1$ for $\introN$ with respect to the morphism $\introN_t$. On the other hand, $\introN_{\beta(t)}$ is the coalgebra morphism in $Z$ from $\beta(t) = H(\introN_t)$ to $4_{H(t)} = F'_2 = H(\introN_{\Box_T t})$ whose underlying morphism in $C'_1$ is itself $\beta(t)$. Thus, $\beta(\introN_t)$ and $\introN_{\beta(t)}$ are both the same, both being the following diagram:
...
This completes the demonstration that $\beta$ is an introspective theory homomorphism, and thus completes the proof of the desired terminality property.
\end{proof}
...
\end{construction}
\openDetails
For convenience, we will say simply \quote{lexfunctor} in the following to mean \quote{strict lexfunctor}.

Let $\gamma = \langle C'_1, C'_2, C'_3, \ldots; F'_1, F'_2, F'_3, \ldots \rangle$. Throughout the following, let unprimed names of lexcategories denote global aspects of primed names and let the correspondence between primed and unprimed names of lexfunctors be as before as well.
