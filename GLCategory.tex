...
This is by exactly the same arguments as we have just given. All the results and arguments given earlier in this chapter apply just as well mutatis mutandis when lexcategories and lexfunctors are replaced by gadgets and gadget homomorphisms, except for \magicref{IntrospAsGeminal} (it will not be the case that an arbitrary strict introspective theory can be viewed as a geminal gadget). However, the analogue of the construction \magicref{IntrospContainsGeminal} still holds (i.e., given an introspective theory $\langle T, C \rangle$ such that $C$ is the underlying lexcategory of a gadget, then the geminal category structure $C$ is equipped with by \magicref{IntrospContainsGeminal} furthermore underlies geminal gadget structure).
...
\sTODOinline{Establish a general property that mapping from Cn to C(n - 1) to Cn matches mapping from Cn to C(n + 1) to Cn, in a geminal category, where the maps to lower indices are given by Gamma and the maps to higher indices are given by F. This is, I think, straightforwardish if we just diagram chase a generic element of Mor(Cn)?}
}
