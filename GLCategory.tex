...
Observe that $\Box_{C_1} \circ H = H \circ \Box_T$, via the following commutative diagram:
...
In the above diagram, the left side is the definition of $\Box_T$ and the right side is the definition of $\Box_{C_1}$. The top rectangle is by $H$ being a geminal category homomorphism and the bottom rectangle is by \magicref{InducedGlobalCommute}.
...
\TODOinline{Show a similar result for how to take a lexcategory with an internal geminal category, and find a terminal geminal category mapping into this lexcategory in such a way as takes its own internal geminal category to this one}.

\TODOinline{Putting together the above two constructions should give the following:}

----

Let $\langle T, C, \introS, \introN \rangle$ be a \TODOinline{strict?} introspective theory. Let the endolexfunctor $\sigma$ on $T \times \Glob{C}$ be defined by $\sigma(t, c) = (\Gamma_C(c), N(c))$, where the endolexfunctor $N : \Glob{C} \to \Glob{C}$ is given by considering the internal lexfunctors $\introN_C : C \to \Gamma_C(\introS[C])$ and $\Hom_{\introS[C]}(1, -) : \Gamma_C(\introS[C]) \to C$, composing these to get an internal endolexfunctor on $C$, and taking its global aspect.

We have a natural transformation $\rho : \sigma \to \sigma^2$ given by \TODO.

Because $\sigma$ is a lexfunctor, the category of $\sigma$-coalgebras is a lexcategory (by similar reasoning to \magicref{CommaCategoryColimits}; indeed, this can be seen as a (non-full) sublexcategory of the comma category $\comma{\id}{\sigma}$).

Let $\beta : T \to T \times \Glob{C}$ be the lexfunctor whose first projection is $\id_T$ and whose second projection is $\introS$. Observe that there is a natural transformation from $\beta$ to $\sigma \circ \beta$ \TODOinline{the first projection of this is $\introN$ and the second projection of this is given by $\introS[\introN]$}. This acts as a lexfunctor from $T$ to the lexcategory of $\sigma$-coalgebras. The coalgebras $m$ in the range of this lexfunctor always satisfy the property that the following diagram commutes \TODOinline{Verify this, and define $\rho$}:

% https://q.uiver.app/?q=WzAsNCxbMCwwLCJ4Il0sWzAsMiwiXFxzaWdtYSh4KSJdLFsyLDAsIlxcc2lnbWEoeCkiXSxbMiwyLCJcXHNpZ21hXjIoeCkiXSxbMCwxLCJtIiwyXSxbMCwyLCJtIl0sWzIsMywiXFxzaWdtYShtKSJdLFsxLDMsIlxccmhvX3giLDJdXQ==
\[\begin{tikzcd}
	x && {\sigma(x)} \\
	\\
	{\sigma(x)} && {\sigma^2(x)}
	\arrow["m"', from=1-1, to=3-1]
	\arrow["m", from=1-1, to=1-3]
	\arrow["{\sigma(m)}", from=1-3, to=3-3]
	\arrow["{\rho_x}"', from=3-1, to=3-3]
\end{tikzcd}\]

Now, let $K$ be a lexcategory, and let $M : T \to K$ be a lexfunctor. Let the endolexfunctor $\lambda$ on $K \times \Glob{M[C]}$ be defined by $\lambda(k, mc) = (\Gamma_{M[C]}(mc), ...)$. \TODOinline{Precisely analogous to the above}.

Observe that we have a lexfunctor $J : T \times \Glob{C} \to K \times \Glob{M[C]}$ given by $M \times \InducedHomo{M}{C}$.

Observe that $J \circ \sigma = \kappa \circ J$ \TODOinline{By straightforward calculation}.

Thus, $J$ also acts as a lexfunctor from the lexcategory of $\sigma$-coalgebras to the lexcategory of $\lambda$-coalgebras. Together with our previous observation, this gives us a lexfunctor from $T$ to the lexcategory of $\lambda$-coalgebras, such that the objects in the range of this lexfunctor always satisfy the property \TODOinline{Precisely analogous to the above}.

\TODOinline{Observe that this category of suitable coalgebras can be equipped as an introspective theory, and this lexfunctor becomes an introspective theory homomorphism. Observe the uniqueness property of this introspective theory homomorphism, and that this construction only depends on the geminal category structure (not on any other aspects of $T$ or $M$), thus making this a suitably co-free introspective theory.}

\begin{TODOblock}
The above is a cleaner account of the following:
