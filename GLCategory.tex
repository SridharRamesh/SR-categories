...
But all of the aforementioned structure follows from just the existence of $C$, $C'$, $\glQuote$, $\glQuote'$, and the equations $\glQuote \circ \glQuote = \glQuote' \circ \glQuote$ and $\glQuote' \circ \glQuote' = \glQuote'' \circ \glQuote'$. \TODOinline{As each $n + 2$ structure or property is given by $(n + 1)$-fold application of $\glQuote$ to the corresponding $n = 1$ structure or property, using the fact that $\glQuote$ and $\introF$ have the same action.}
...
\TODOinline{Make the observations about where the necessity of strict structure, preservation of limits on the nose, etc, come in}

\TODOinline{Write up all the details here, and word it all better, more followably. This whole chapter is perhaps the prime example of the sort of thing which had previously been considered a mess in the 2016 notes. Well, who knows?}
\end{construction}
