...
Let $\langle T, C, \introS, \introN \rangle$ be a strict introspective theory. Let the endolexfunctor $\sigma$ on $T \times \Glob{C}$ be defined by $\sigma(t, c) = (\Gamma_C(c), N(c))$, where the lexfunctor $N : \Glob{C} \to \Glob{C}$ is given by considering the internal lexfunctor $\introN_C : C \to \Gamma_C(\introS[C])$ and \TODO.

Observe that we have a lexfunctor from $T$ to the category of $\sigma$-coalgebras. \TODO. The objects in the range of this lexfunctor always satisfy the property \TODO.

Now, let $K$ be a lexcategory, and let $M : T \to K$ be a lexfunctor. Let the endolexfunctor $\lambda$ on $K \times \Glob{M[C]}$ be defined by $\lambda(k, mc) = (\Gamma_{M[C]}(mc), ...)$. \TODO.

Observe that we have a lexfunctor $J : T \times \Glob{C} \to K \times \Glob{M[C]}$ given by $M \times \InducedHomo{M}{C}$.

Observe that $J \circ \sigma = \kappa \circ J$??? \TODO.

Thus, the category of $\sigma$-coalgebras is transported by $J$ to the category of $\lambda$-coalgebras. \TODO. Together with our previous observation, this gives us a lexfunctor from $T$ to the category of $\lambda$-coalgebras, such that the objects in the range of this lexfunctor always satisfy the property \TODO.

\TODOinline{Observe that this category of suitable coalgebras can be equipped as an introspective theory, and this lexfunctor becomes an introspective theory homomorphism.}
