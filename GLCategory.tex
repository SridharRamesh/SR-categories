...
\begin{itemize}
    \item 
     We require that $\straight{F}_i[C'_j] = C'_{j + 1}$ and $\straight{F}_i[F'_j] = F'_{j + 1}$ for $j > i \geq 1$.

     (Note that this is referring to the induced action of $\straight{F} : C_i \to C_{i + 1}$ in taking structures $(j-i)$-tuply internal to $C_i$ to structures $(j - i)$-tuply internal to $C_{i + 1}$.)
