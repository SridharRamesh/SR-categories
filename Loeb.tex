...
\begin{theorem}\label{VeryGeneralSelfRelated}
Let $T$ be an arbitrary category, and let $\Unipresheaf \in \Psh{T}$ and $\Bipresheaf \in \Psh{T \times T}$. Suppose given also a map $\omega : \pullAlong{\Delta}\Bipresheaf \to \Unipresheaf$ where $\Delta : T \to T \times T$ is the diagonal functor. (That is, $\omega$ comprises a system of maps $\omega_t : \Bipresheaf(t, t) \to \Unipresheaf(t)$ for each object $t$ in $T$, natural in $t$).

Also, suppose given an object $\point$ in $T$ and a binary relation $R$ on the set $\Unipresheaf(\point)$. (We do not presume $R$ to be an equivalence relation or symmetric or any such thing.)

Let us now suppose given an object $\surjSource$ of $T$ and an $\App \in \Bipresheaf(\surjSource, \surjSource)$ such that $\App$ has the pointwise surjectivity-like property that, for every $F \in \Unipresheaf(\surjSource)$, there is an $f \in \Hom(\point, \surjSource)$ such that for every $x \in \Hom(\point, \surjSource)$, we have that $\omega_{\point}( \App(f, x) ) \in \Unipresheaf(\point)$ is related by $R$ to $F(x) \in \Unipresheaf(\point)$.

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	& {(\surjSource, \surjSource)} \\
	{(\point, \point)} && \Bipresheaf
	\arrow["\App", from=1-2, to=2-3]
	\arrow["{(f, x)}", from=2-1, to=1-2]
	\arrow[""{name=0, anchor=center, inner sep=0}, "{\App(f, x)}"', from=2-1, to=2-3]
	\arrow[shorten <=3pt, Rightarrow, no head, from=0, to=1-2]
\end{tikzcd}\]

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	\point && \Unipresheaf \\
	& \surjSource
	\arrow[""{name=0, anchor=center, inner sep=0}, "{\omega_{\point}(\App(f, x))}", from=1-1, to=1-3]
	\arrow["x"', from=1-1, to=2-2]
	\arrow["F"', from=2-2, to=1-3]
	\arrow["R", shorten <=3pt, Rightarrow, squiggly, from=0, to=2-2]
\end{tikzcd}\]

Then there is an element of $\Unipresheaf(\point)$ which is related by $R$ to itself.
\end{theorem}
\begin{proof}
Take the pointwise surjectivity-like supposition and consider the particular case where $F = \omega_{\surjSource}(\App) \in \Unipresheaf(\surjSource)$ and $x = f$. This tells us that $\omega_{\point}( \App(f, f) )$ is related by $R$ to $\omega_{\surjSource}(\App)(f)$. But by the naturality of $\omega$ with respect to $f$, we have that $\omega_{\surjSource}(\App)(f) = \omega_{\point}( \App(f, f) )$ as well. Thus, this is a value which is related to itself.

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	{\Bipresheaf(\surjSource, \surjSource)} &&& {\Bipresheaf(\point, \point)} \\
	& \App & {\App(f, f)} \\
	& {\omega_{\surjSource}(\App)} & {\omega_{\surjSource}(\App)(f) = \omega_{\point}(\App(f, f))} \\
	{\Unipresheaf(\surjSource)} &&& {\Unipresheaf(\point)}
	\arrow["{\omega_{\surjSource}}"', from=1-1, to=4-1]
	\arrow["{\Bipresheaf(f, f)}", from=1-1, to=1-4]
	\arrow["{\Unipresheaf(f)}"', from=4-1, to=4-4]
...
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	\point && \Unipresheaf \\
	& \surjSource
	\arrow[""{name=0, anchor=center, inner sep=0}, "{\omega_{\point}(\App(f, f))}", from=1-1, to=1-3]
	\arrow["f"', from=1-1, to=2-2]
	\arrow["{\omega_{\surjSource}(\App)}"', from=2-2, to=1-3]
	\arrow["R", shift left=2, shorten <=3pt, Rightarrow, squiggly, from=0, to=2-2]
	\arrow[shift right=2, shorten <=3pt, Rightarrow, no head, from=0, to=2-2]
\end{tikzcd}\]
This completes the proof.

\TODOinline{Notationally, we should perhaps write $\pullAlong{f}\omega$ instead of $\omega(f)$ and the like.}
\end{proof}

We have given our generalization of Lawvere's fixed point theorem in such a general form above for comparison to other generalizations of Lawvere's fixed point theorem. See the related work section at the end. However, we will now specialize this generalization back down to a more specific form, more suited to our particular needs in thinking about introspective theories and their brethren.
...
First observe that the presheaf action of $P$ gives us a map $: P(\introS(t')) \times \Hom_C(1, \introS(t')) \to P(1)$ extranatural in $t'$, and therefore, by applying $\Hom(t, -)$, we have a transformation $: \Hom(t, P(\introS(t'))) \times \Hom(t, \Hom_C(1, \introS(t'))) \to \Hom(t, P(1))$, separately natural in $t$ and extranatural in $t'$. We specialize $t' = t$ and we compose onto the second factor of the input the extranatural transformation $: 1 \to \Hom(t, \Hom_C(1, \introS(t)))$ induced by $\introN$, to get our natural transformation $\omega_t : \Hom(t, P(\introS(t))) \to \Hom(t, P(1))$.
...
Thus, we have shown that $\omega_{\point} (\App(f, x))$ is related by $E_{\point}$ to $\pullAlong{\introN_{P(1)}(F(x))}j$. This means $\omega_{\point} (\App(f, x))$ is related by $R$ to $F(x)$. This was exactly the pointwise surjectivity presumption we needed to establish in order to invoke \magicref{VeryGeneralSelfRelated} and so we are now done. Note that the conclusion of this invocation of \magicref{VeryGeneralSelfRelated} is exactly the conclusion we sought to establish.
