...
Consider now a $T$-indexed almost-presheaf $P$ on $C$ \TODOinline{We will need to clarify in the preliminaries what this amounts to, in the double-indexing section or some such place. We mean a presheaf along with an equivalence relation such that pullback isn't strictly functorial, but only functorial up to the relation, and also is relation-preserving. The equivalence relation itself does not need to satisfy any coherences.}. Suppose also given some object $X$ in $T$ and map $\alpha : X \to P(\introS(X))$.
...
Now, let us make a surjectivity-like assumption on $\alpha$. Specifically, suppose we are given a relation $E$ on $P$ \TODOinline{Actually, let us suppose in particular that $E$ is the equivalence relation of the almost-presheaf $P$?}, and let us suppose that for every global element of $P(\introS(X))$, there is a point of $X$ such that \TODO.
...
If we define $E_j(\omega_1, \omega_2)$ as $E(\omega_1, \pullAlong{\introN_{\Omega}(\omega_2)} j)$ accordingly, we have shown the surjectivity requirement to invoke \magicref{SelfRelatedPointTheorem} with $R = E_j$, and thus we get a point which is related by $E_j$ to itself, which is what we desired.
...
\TODOinline{Perhaps first give a proof of the introspective theory diagonalization for strict presheaves with no imposed relation, then give the relational version}
