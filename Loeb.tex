...
Let $T$ be an arbitrary category, and let $P_1 \in \Psh{T}$ and $P_2 \in \Psh{T \times T}$. Suppose given also a map $\omega : \pullAlong{\Delta}P_2 \to P_1$ where $\Delta : T \to T \times T$ is the diagonal functor. (That is, $\omega$ comprises a system of maps $\omega_t : P_2(t, t) \to P_1(t)$ for each object $t$ in $T$, natural in $t$).

Also, suppose given an object $point$ in $T$ and a binary relation $R$ on the set $P_1(point)$. (We do not presume $R$ to be an equivalence relation or symmetric or any such thing.)

Let us now suppose given an object $s$ of $T$ and an $App \in P_2(s, s)$ such that $App$ has the pointwise surjectivity-like property that, for every $F \in P_1(s)$, there is an $f \in \Hom(point, s)$ such that for every $x \in \Hom(point, s)$, we have that $\omega_{point}( App(f, x) ) \in P_1(point)$ is related by $R$ to $F(x) \in P_1(point)$.

% https://q.uiver.app/?q=WzAsMyxbMSwxLCIocywgcykiXSxbMiwwLCJQXzIiXSxbMCwwLCIocG9pbnQsIHBvaW50KSJdLFswLDEsIkFwcCIsMl0sWzIsMCwiZiBcXHRpbWVzIHgiLDJdLFsyLDEsIkFwcChmLCB4KSJdLFs1LDAsIiIsMCx7InNob3J0ZW4iOnsic291cmNlIjoyMH0sInN0eWxlIjp7ImhlYWQiOnsibmFtZSI6Im5vbmUifX19XV0=
\[\begin{tikzcd}
	{(point, point)} && {P_2} \\
	& {(s, s)}
	\arrow["App"', from=2-2, to=1-3]
	\arrow["{f \times x}"', from=1-1, to=2-2]
	\arrow[""{name=0, anchor=center, inner sep=0}, "{App(f, x)}", from=1-1, to=1-3]
	\arrow[shorten <=3pt, Rightarrow, no head, from=0, to=2-2]
\end{tikzcd}\]

% https://q.uiver.app/?q=WzAsMyxbMCwwLCJwb2ludCJdLFsyLDAsIlBfMSJdLFsxLDEsInMiXSxbMCwxLCJcXG9tZWdhX3twb2ludH0oQXBwKGYsIHgpKSJdLFswLDIsIngiLDJdLFsyLDEsIkYiLDJdLFszLDIsIlIiLDAseyJzaG9ydGVuIjp7InNvdXJjZSI6MjB9LCJzdHlsZSI6eyJib2R5Ijp7Im5hbWUiOiJzcXVpZ2dseSJ9fX1dXQ==
\[\begin{tikzcd}
	point && {P_1} \\
	& s
...
Then there is an element of $P_1(point)$ which is related by $R$ to itself.
\end{theorem}
\begin{proof}
Specifically, take the supposition two sentences ago and consider the particular case where $F = \omega_s(App) \in P_1(s)$ and $x = f$. This tells us that $\omega_{point}( App(f, f) )$ is related by $R$ to $\omega_s(App)(f)$. But by naturality, $\omega_s(App)(f) = \omega_{point}( App(f, f) )$ as well. Thus, this is a value which is related to itself.

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	...
	\arrow["{P_2(f, f)}", from=1-1, to=1-4]
	\arrow["{P_1(f)}"', from=4-1, to=4-4]
...
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	point && {P_1} \\
	& s
	\arrow[""{name=0, anchor=center, inner sep=0}, "{\omega_{point}(App(f, f))}", from=1-1, to=1-3]
	\arrow["f"', from=1-1, to=2-2]
	\arrow["{\omega_s(App)}"', from=2-2, to=1-3]
...
This leads immediately to a generalization of itself as corollary, which is most often the form we are interested in:

\begin{corollary}\label{VeryGeneralFixedModuloRelation}
Let all values be given as in \magicref{VeryGeneralSelfRelated}. Furthermore, suppose we have some additional relation $S$ on $P_1(point)$ such that for every $F \in P_1(s)$, there is some $G \in P_1(s)$, such that for every $x : point \to s$, we have that $F(x)$ and $G(x)$ are related by $S$. Then there are a pair of elements $x, y \in P_1(point)$ such that $x$ and $y$ are simultaneously related by both $R$ and $S$.
\end{corollary}
\begin{proof}
Consider the relation $RandS(x_1, x_2) = $ \quote{There exists a $y$ such that $R(x_1, y)$ and $S(x_2, y)$}. Now suppose given an arbitrary $F \in P_1(s)$ and let $G \in P_1(s)$ be chosen accordingly in keeping with our presumption. By the presumed pointwise surjectivity of $App$ with respect to $R$ as applied to $G$, there is an $f \in \Hom(point, s)$ such that for every $x \in \Hom(point, s)$, we have that $\omega_{point}( App(f, x) ) \in P_1(point)$ is related by $R$ to $G(x) \in P_1(point)$. As $F(x)$ is automatically related by $S$ to $G(x)$, it follows that $\omega_{point}( App(f, x) )$ is related by $RandS$ to $F(x)$. This demonstrates the pointwise surjectivity of $App$ with respect to $RandS$ for arbitrary $F$. Thus, we can invoke \magicref{VeryGeneralSelfRelated} with $RandS$ in place of $R$, to obtain some $x$ which is related by $RandS$ to itself. Which is to say, there is some $y$ such that $x$ is related to $y$ by both $R$ and $S$, as desired.
\end{proof}

The corollary we will need will be more suited to our use of introspective theories. It is like so:

\begin{corollary}
Let $\langle T, C, \introS, \introN \rangle$ be a pre-introspective theory with finite products.\footnote{We do not actually use any of the finite product structure in this proof, beyond the assumption that the $D$-defined aspect of $C$ has some particular object $1$ available to us. Even the terminality of this object $1$ is not important in the proof. But we state things in terms of pre-introspective theories with finite products anyway, as every application we are interested in has this structure.}
...
Then from any $D$-defined generalized element $f$ of $P(\introS(P(1)))$, we obtain a $D$-defined element $L(f)$ of $P(1)$, such that transporting $f$ back along $\introN_{P(1)}(L(f))$ yields a value related by $R$ to $L(f)$.
\end{corollary}
\begin{proof}
This is the instance of \magicref{VeryGeneralFixedModuloRelation} where we take $P_2(t_1, t_2) = \Hom(t_1, P(\introS(t_2)))$ and $P_1(t) = \Hom(t, P(1))$. Furthermore, $\omega_t(b)$ is given by taking $\introN_t : t \to \Hom(1, \introS(t))$ and combining it with $b : t \to P(\introS(t))$ using the presheaf action of $P$, to get $\omega_t(b) : t \to P(1)$. Finally, we take $n$ to be given by \TODO. The relational surjectivity is ensured by \TODO.
\end{proof}
...
\end{corollary}
\begin{proof}
This is the instance of \magicref{VeryGeneralSelfRelated} where we take $P_2(t) = \Hom(B(t, t), p)$, $P_1(t) = \Hom(t, p)$, and $\omega_t(-) = - \circ \delta_t$, and we take $R(a, b)$ to be that $b$ equals $m \circ a$.
...
Many of the other theorems in the David Michael Roberts paper can also be seen as instances of our \magicref{VeryGeneralSelfRelated}. However, I have not been able to see conversely that our next theorem is an instance of any of the David Michael Roberts theorems (though of course the possibility exists t), even the special case of our next theorem when $R$ is restricted to identity. Nor conversely have I been able to establish the David Michael Roberts theorem from our next theorem. These appear to be incomparable points in the space of theorems between Lawvere's fixed point theorem and our very general \magicref{VeryGeneralSelfRelated}.
...
\TODOinline{Discuss the relationship to models of guarded recursion, and the Birkedal et al paper on \quote{First steps in synthetic...}, where the initiality/terminality argument for contractive functors is presented}
