...
Next we apply \magicref{SelfIndexingIsFree}. The map from $T/-$ to $C$ as $T$-indexed (lexcategories under $T$) is the same as choosing, in a natural way over all $t$ in $T$, some $t$-defined value in $\Hom_C(1, \introS(t))$. That is, a natural transformation from $t$ to $\Hom_C(1, \introS(t))$.
\end{proof}

\begin{remark}\label{IntrospGeneralDoctrine}
It wasn't fundamentally important that we were dealing with lexcategories here. The use of \Cref{Lemma1} as applied to $\op{C}$ only required a terminal object in $C$. And for any algebraish notion extending categories-with-global-elements, there is some free construction $T[1 \to -]$. (Even the role terminality plays here is to some degree eliminable, though we have no interest for now in eliminating it). In particular, we get a completely analogous result if we replace throughout all references to lexcategories with references to categories with finite products. The same also holds for for any notion extending the notion of lexcategories which is automatically inherited by slice categories and preserved by pullback, or any notion extending the notion of finite-product-categories which is automatically inherited by these reduced slice categories and preserved by their pullbacks (a la \magicref{SelfIndexingIsFreeCorollary}). Even infinitary notions could be used; categories with all limits of cardinality below some regular cardinal, say.
\end{remark}
...
\TODOinline{This is actually an initial algebra catamorphism, keep in mind.}
...
\TODOinline{Rename variables above, using $Th(ZF-Finite_{\Sigma_1})$, etc, instead of T}
