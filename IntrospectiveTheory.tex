...
\begin{observation}\label{ReindexSmallAdjoint}
If $C$ is a $T$-indexed structure, and $\Sigma : U \to T$ is any functor, then $C \Sigma$ is a $U$-indexed structure, like so:

% https://q.uiver.app/?q=WzAsMyxbMCwwLCJcXG9we1V9Il0sWzIsMCwiXFxvcHtUfSJdLFs0LDAsIlxcYnVsbGV0Il0sWzAsMSwiXFxvcHtcXFNpZ21hfSJdLFsxLDIsIkMiXV0=
\[\begin{tikzcd}
	{\op{U}} && {\op{T}} && \bullet
...
Furthermore, if $\Sigma$ has a right adjoint, then if $C$ is small or locally small, then so respectively will be $C \Sigma$, as the representing objects for $C \Sigma$ will simply be the image of those for $C$, as mapped into $U$ via $\Sigma$'s right adjoint. \TODOinline{Make this whole observation in the preliminaries. Also, write out that this right adjoint business really works for maps with small fibers/locally small structures. Note that the definition of having small fibers only makes proper sense when indexed over a category with pullbacks.}
\end{observation}
...
By \cref{{ReindexSmallAdjoint}}, if $\Sigma$ has a right adjoint, we can further observe that if $C$ is small or locally small, then so respectively will be $C \Sigma$.
\end{proof}
