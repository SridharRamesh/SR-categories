...
\end{construction}
\begin{proof}[Details]
Consider the forgetful functor $\Sigma : T/t \to T$ (i.e., the domain projection functor). Composition and whiskering with this gives us the triple $\langle T/t, C \Sigma, \introF \Sigma \rangle$, like so:
...
When $C$ is small or locally small, then so respectively is $C \Sigma$. The representing objects for $C \Sigma$ will simply be those for $C$, mapped into $T/t$ via $\Sigma$'s right adjoint (i.e., pulled back along the unique morphism from $t$ to $1$).
\end{proof}

% It is perhaps a little confusing that we use $\sigma$ here both for the map from $T/t$ to $T$ and the map from $\op{T/t}$ to $\op{T}$. In particular, when we speak of its right adjoint, we mean the right adjoint in the former sense of $\Sigma$, but this may be confusing since we're really using the latter sense of $\Sigma$, with respect to which the adjoint we are talking about is the left adjoint.
...
\end{construction}
\begin{proof}[Details]
% https://q.uiver.app/?q=
\[\begin{tikzcd}
	{\op{S}} && {\op{T}} && \LexCat
	...
	\arrow[""{name=2, anchor=center, inner sep=0}, "{S/-}", shift left=5, curve={height=-18pt}, from=1-1, to=1-5]
	\arrow["\introF"', shorten <=1pt, shorten >=1pt, Rightarrow, from=0, to=1]
	\arrow[shorten <=3pt, Rightarrow, from=2, to=1-3]
\end{tikzcd}\]

The unlabelled arrows here are inclusions. Note that the inclusion of $S$ into $T$ followed by the self-indexing $T/-$ does not yield the same thing as the self-indexing $S/-$; however, the latter is canonically included in the former (as an indexed full sub-lexcategory).
\end{proof}
