...
It is sometimes useful to relax all finite limit structure in \magicref{DefnPreIntrospSN} to merely finite product structure:

\begin{definition}
A \defined{pre-introspective finite product theory} is a finite product category $T$, a $T$-indexed finite product category $C$, a finite product preserving functor $\introS$ from $T$ to the global aspect of $C$, and a natural transformation $\introN$ from $t$ in $T$ to $\Hom_C(1, \introS(t))$.
\end{definition}

As ever, we may write out $\langle T, C, \introS, \introN \rangle$ or $\langle T, C, \introS, \introN, 1 \rangle$ to be fully explicit, but in typical abuse of language, will usually say $\langle T, C \rangle$ to describe a pre-introspective finite product theory. Of course, every pre-introspective theory is a fortiori a pre-introspective finite product theory using the same components.

Just as for pre-introspective theories, these admit an alternative definition along the lines of \magicref{DefnPreIntrospIndexed}, with the equivalence between the two definitions established by the remark after \autoref{SNCorrespondence}:

\begin{definition}
A \defined{pre-introspective finite product theory} is a finite product category $T$, a $T$-indexed finite product category $C$, and a finite product preserving functor $\introF$ from the simple self-indexing of $T$ to $C$, like so:

% https://q.uiver.app/?q=WzAsMixbMCwwLCJcXG9we1R9Il0sWzIsMCwiXFxGaW5Qcm9kQ2F0Il0sWzAsMSwiVC8vLSIsMCx7Im9mZnNldCI6LTJ9XSxbMCwxLCJDIiwyLHsib2Zmc2V0IjoyfV0sWzIsMywiXFxpbnRyb0YiLDIseyJzaG9ydGVuIjp7InNvdXJjZSI6MjAsInRhcmdldCI6MjB9fV1d
\[\begin{tikzcd}
	{\op{T}} && \FinProdCat
	\arrow[""{name=0, anchor=center, inner sep=0}, "{T//-}", shift left=2, from=1-1, to=1-3]
	\arrow[""{name=1, anchor=center, inner sep=0}, "C"', shift right=2, from=1-1, to=1-3]
...
It is natural now also to generalize \autoref{DefnLocallyIntrosp}:
\begin{definition}
By a \defined{locally introspective finite product theory}, we mean a pre-instrospective finite product theory $\langle T, C \rangle$ for which $C$ is locally \repsmall/.
\end{definition}

However, we do not take any special interest in locally introspective finite product theories $\langle T, C \rangle$ for which $C$ is furthermore fully \repsmall/, for reasons connected to the footnote given in \magicref{IndexedStructuresDefn}. Thus, the concept of an introspective theory does not have a complete analogue in the finite product context.

Returning to the world of finite limits, there is one final definition we will need for technical reasons later on. Recall that a small indexed lexcategory is one which is equivalent to some small indexed strict lexcategory (with possibly multiple non-isomorphic such choices available). It is occasionally of use to imagine some particular such choice has been pinned down, leading to the following definition.
...
\TODOinline{We should probably just define introspective theories in the first place as inner-strict introspective theories. This makes everything easier when we discuss homomorphisms, Loeb's theorem, etc. At least, we should make some note about how every construction we give is really a construction of inner-strict introspective theories.}
