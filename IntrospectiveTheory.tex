...
If $\langle T, C, \introS, \introN \rangle$ is an introspective theory, and $\introS : T \to \Glob{C}$ factors through some $f : T \to T[M^{-1}]$ which is a lex localization in the sense of \magicref{DefnLexLocalization}, then the localization functor $f$ acts as an introspective theory homomorphism from $T$ to a uniquely determined introspective theory $\langle T[M^{-1}], f(C), \ldots \rangle$.
\end{construction}
\begin{definition}
If $f$ is to act as an introspective theory homomorphism, it must be to some introspective theory of the form $\langle T', C', \introS', \introN' \rangle$. We will show that each of these components are uniquely determined by the requirements of \magicref{StrictIntrospHomoDefn} (\TODOinline{Or rather, its non-strict analogue}).

That $T'$ is uniquely determined as $T[M^{-1}]$ and $C'$ is uniquely determined as $f(C)$ is immediate.
...
The requirement on $\introN'$ is that $\introN'$ whiskered along $f$ is $f$ whiskered along $\introN$ from the other side. By \magicref{LocalizingTransform}, this uniquely determines $\introN'$.
\end{definition}
