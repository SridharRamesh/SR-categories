...
Furthermore, if $\Sigma$ has a right adjoint, then if $C$ is small or locally small, then so respectively will be $C \Sigma$, as the representing objects for $C \Sigma$ will simply be the image of those for $C$, as mapped into $U$ via $\Sigma$'s right adjoint.
\end{observation}
