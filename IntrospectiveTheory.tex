...
% Non-evil definition
\subsection{First definition (indexed)}
...
\begin{example} \label{TrivialIntrosp}
...
\subsection{Second definition (non-indexed)}
...
If $\langle T, C \rangle$ is a pre-introspective theory and $C$ is small or locally small, then we say this is an \defined{introspective theory} or \defined{locally introspective theory}, respectively. And in the same way, for any of the generalized notions of pre-introspective X theory, we may replace \quote{pre-introspective} with \quote{introspective} or \quote{locally introspective} to designated that $C$ is small or locally small, respectively.\TODOinline{It is usually only worth discussing introspective X theories such that X-homorphisms preserve the structure of Xes internal to Xes (in particular, X must extend the concept of lex categories), or locally introspective X theories such that X-homomorphisms preserve the structure of an X enriched over an X (in particular, X must extend the concept of a category with finite products). This all may therefore be too much or too unwieldy generality to introduce. We really just need the cases where X is lex, finite product, or unary. There is also a comment about the case where X is (locally) cartesian closed, at the end of this chapter.}
\end{definition}
...
Let us now finally give the example-oriented reader an archetypal example of an introspective theory by which to orient themselves. The following construction will be discussed in more detail later at \cref{Sigma1Model}.
\end{construction}

Let us start with the first-order logical theory ZF-Finite: the theory ZF but with the axiom of infinity replaced by its negation\footnote{This theory happens to be equivalent to Peano Arithmetic in a suitable sense, but it will be more convenient for us to speak in terms of ZF-Finite so as not to fret about codings of a sort every modern mathematician readily takes for granted in a ZF-style context.}. The universe of this theory amounts to the collection of all hereditarily finite sets. Throughout the following, whenever we speak of formulae, we mean formulae in the language of ZF-Finite, and whenever we speak of provability, we mean provable within ZF-Finite.

Certain formulae are $\Sigma_1$. These are the formulae which consist of an initial string of unbounded existential quantifiers, after which all other quantifiers are bounded (ranging only over finite sets that have previously been introduced).
...
Its objects are the $\Sigma_1$ binary relations $\phi(n, m)$ which are provably partial equivalence relations (i.e., symmetric and transitive), thus corresponding to certain subquotients of the universe.

Given any two such formulae $\phi(n, m)$ and $\psi(n, m)$, a morphism in $T$ from $\phi$ to $\psi$ is a $\Sigma_1$ formula $F(n, m)$ which provably corresponds to the graph of a function between the corresponding subquotients of the universe. That is, such that the universal closures of all the following are provable:
...
Given morphisms $F : \phi \to \psi$ and $G: \psi \to \chi$ of this sort, we again define their composition in the usual way of composing functions represented as graphs, as $(F \circ G)(n, m) = \exists p [G(n, p) \wedge F(p, m)]$.

This all describes the category $T$, which one can verify is indeed a category and moreso, a category with finite limits.

Note that our construction of $T$ is such that the objects of $T$, the morphisms of $T$, the equivalence relation on morphisms of $T$, the composition structure of $T$, the finite limit structure of $T$, etc, are all definable within the language of ZF-Finite; indeed, all definable by $\Sigma_1$ formulae. Thus, there is a lexcategory $D$ internal to $T$ which corresponds to this very same construction we have just described. And we have a lexfunctor $\introF$ from $T$ to the global aspect of $D$ which sends each piece of the construction of $T$ to the corresponding piece of the construction of $D$. This is all straightforward.

And we even have a natural transformation $\introN$ from the identity endofunctor on $T$ to the endofunctor $\Hom_D(1, \introF(-))$ on $T$. Essentially, to every hereditarily finite set $x$, we can assign it a code $\code{x}$, which is an explicit term in the language of ZF-Finite denoting that set. The easy way to do this is to recursively assign to each set $\{a, b, c, \ldots\}$ the term describing a finite set whose members are explicitly enumerated by the terms assigned to $a, b, c, \ldots$. We thus send a set such as $\{\{\}, \{\{\}\}\}$ to the term in the language of ZF-Finite which might be called \quote{$\{\{\}, \{\{\}\}\}$}, and so on.

This gives us a function $\code{-}$ from hereditarily finite sets to terms in the language of ZF-Finite which describe hereditarily finite sets. This function $\code{-}$ is itself definable by a $\Sigma_1$ formula and thus gives a morphism in $T$, which serves as the component of $\introN$ at the object $V$ of $T$ describing ALL hereditarily finite sets. 

The other objects of $T$ are subquotients of that object (and similarly for the objects of $D$), and therefore the components of the natural transformation $\introN$ at those other objects are uniquely determined from $\introN_{V}$ as certain factorizations, supposing those factorizations to exist at all. And because of the $\Sigma_1$-ness of the objects of $T$, these factorizations will indeed all exist, such that $\introN$ is fully defined. The $\Sigma_1$-ness is important here, however! The $\Sigma_1$-ness of a formula $\phi$ guarantees that whenever $x$ satisfies $\phi$, the term $\code{x}$ provably satisfies $\phi$ (because the fact that $x$ satisfies $\phi$ is witnessed by particular choices for the unbounded existential quantifiers in $\phi$, and those witnesses can themselves be encoded as terms by $\code{-}$, which can be used to construct a proof in ZF-Finite of $\phi(\code{x})$). If this had failed, we would not have the appropriate factorization.

More generally, the naturality squares for $\introN$ in general depend on the morphisms in $T$ being given by $\Sigma_1$ formulae, again because of the property that when such formulae are true, they are provably true. \TODOinline{Write this out more clearly}

Thus, we obtain an introspective theory $\langle T, D, F, N \rangle$.

However, $\langle T, D, F, N \rangle$ is not actually the most typical introspective theory! It has special properties which we should not expect of a general introspective theory. Its internal $D$ acts as a perfect mirror image of $T$, and can thus itself be equipped as an internal introspective theory. The internal $D$ has in some sense no further objects (or morphisms, or equations) outside the range of $F$. All of this is not typical for introspective theory.
...
Just as the construction of $T$ could itself be carried out in ZF-Finite, to get a $D$ internal to $T$, so too can the construction of $S$ can be carried out in ZF-Finite, to get a $C$ internal to $T$. Yes, this $C$ is internal to $T$, not just internal to $S$! Even though $S$ includes as its objects and morphisms formulae which are not $\Sigma_1$, the description of $S$ (as a category whose objects are symbol-strings for which certain other symbol-strings exist, whose morphisms are symbol-strings for which certain other symbol-strings exist, and so on) is $\Sigma_1$.

Finally, the inclusion of $T$ into $S$ yields, analogously, an inclusion from $D$ into $C$, internal to $T$. This means the functor $\introF$ from $T$ into the global aspect of $D$ can just as well be thought of as having a codomain of $C$, and similarly the natural transformation $\introN$ can just be well as thought of in this context.

In this way, we get an introspective theory $\langle T, C, \introF, \introN$, where $T$ amounts to the $\Sigma_1$-definable sets of hereditarily finite sets and $\Sigma_1$-definable functions between them up to provable equivalence, $C$ is the arbitrary definable sets and arbitrary definable functions between them, $\introF$ assigns to each piece of $T$ the straightforwardly corresponding piece of $C$, and $\introN$ is the definable function which sends a hereditarily finite set to the canonical term describing it, as well as witnessing the entailment from truth to provability for $\Sigma_1$ formulae.

Phew! What a long walk it was to describe that example! All the better, then, that we have formalized introspective theories so abstractly, and can work with them without having to fuss about such concrete details as in that example. But this is indeed the archetypal example it will be best to keep in mind to guide the reader's intuition throughout all further discussion.

\subsection{Inner-strictness}
Returning to our zoo of definitions of slight variants of the concept of an introspective theory, there is one final definition we will need for technical reasons later on. Recall that a small indexed lexcategory is one which is equivalent to some small indexed strict lexcategory (with possibly multiple non-isomorphic such choices available). It is occasionally of use to imagine some particular such choice has been pinned down, leading to the following definition.
