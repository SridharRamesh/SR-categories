...
\TODOinline{Warning to readers: I'm in the middle of modifying this section, so it will be unreadable while I tinker with it.}

In this subsection, we will give some other instructive examples of introspective theories based on presheaf categories. These examples are of a different flavor from those based on logical theories as in the previous section, but thus help illustrate the generality of the notion of introspective theory.

The first example in this section will eventually be used to relate introspective theories to the topos of trees with the \quote{later} modality as used in much work on guarded recursion. This may also be useful to build up intuition as we work towards the more complicated example in this section.

The final examples in this section are also illustrated using the topos of trees, but with a different internal category inducing a different modality. This will eventually be particularly useful for relating introspective theories to the traditional study of \Goedel/-\Loeb/ modal logic via well-founded Kripke frames.
...
\paragraph{Presheaf examples related to the \quote{later} modality from guarded recursion}

We give this example in terms of presheaves over the natural numbers (which comprise the so-called topos of trees), but analogous examples may be constructed for presheaves over arbitrary categories; see \TODOinline{cite later chapter} for a vast generalization. We focus on presheaves over the natural numbers in this example as it is perhaps the simplest nontrivial presheaf category to consider, and also as the \quote{later} modality on the topos of trees which is at the core of this example has already been much considered in work on guarded recursion (a point which we will return to highlighting in \TODOinline{cite later chapter}).

\begin{construction}
Let $\omega$ be the poset of natural numbers with their usual ordering, and consider the category of presheaves $\Psh{\omega}$ (often called the topos of trees). As a topos, this is locally cartesian closed. Accordingly, its self-indexing $\Psh{\omega}/-$ is locally \repsmall/.
...
Let $C$ be the $\Psh{\omega}$-indexed lexcategory given by $C(-) = \Psh{\omega}/\Prior(-)$. That is, $C$ is given by applying $\pullAlong{\Prior}$ to the self-indexing $\Psh{\omega}/-$. Note that $C$ is locally \repsmall/ by \magicref{RepLocallySmallRightAdjoint}, as the self-indexing $\Psh{\omega}/-$ is locally \repsmall/ and $\Prior$ has a right adjoint.

We will now equip $\langle \Psh{\omega}, C \rangle$ as a locally introspective theory, by exhibiting a suitable $\introF$ from $\Psh{\omega}/-$ to $C$. This is via whiskering $\prior$ as in the following diagram:
...
It may be illustrative to alternatively describe this $\introF$ in terms of $\introS$ and $\introN$.
...
This way of viewing $\Psh{\omega}$ as a locally introspective theory is illustrative. Unfortunately, this is not an introspective theory, as our $C = \pullAlong{\Prior}\left(\Psh{\omega}/-\right) \rangle$ is merely locally \repsmall/, not \repsmall/ simpliciter.
...
We might naively try to ameliorate this problem by replacing $\Psh{\omega} = \Set^{\omega}$ by $\left(\Set'\right)^{\omega}$ where $\Set'$ is some full subcategory of $\Set$, such as sets of cardinality below some particular inaccessible cardinal. But it is soon seen that such a uniform cardinality constraint across all $n \in \omega$ will not be workable for fixing the issue.
...
We will describe this process in full generality later at \TODOinline{Cite appropriate later chapter}. For now, we will quickly describe one concrete approach that works:
...
By $\omega_{< n}$, we mean the full subcategory of $\omega$ restricted to those naturals which are less than $n$. By $\PshUnderN{n}$, we mean the full sublexcategory of $\Psh{\omega_{< n}}$ comprising presheaves such that $P(m) \in \Set_m$ for each $m < n$.
...
We also have obvious restriction maps from $\PshUnderN{n}$ to $\PshUnderN{m}$ for $m \leq n$ by projecting away unused components, and any composition of such restriction maps yields the appropriate such restriction map.

Thus, we have an $\omega$-indexed \setsmall/ strict lexcategory $C'(n) = \PshUnderN{n}$. In other words, this is a lexcategory internal to $\Psh{\omega}$.

Also note that once $\Set_0, \Set_1, \ldots, \Set_{n - 1}$ are determined, we have already determined what $\PshUnderN{n}$ is.

Thus, we may readily inductively choose $\Set_n$ for each $n$ such that both $\Ob(\PshUnderN{n})$ and $\Mor(\PshUnderN{n})$ are among the objects of $\Set_n$. 
...
When we choose $\Set_n$ satisfying this condition, we have that $C'$ is not only internal to $\Psh{\omega}$, but indeed lives within its full subcategory $\PshUnder$.
...
\paragraph{Presheaf examples related to the modalities considered in Kripke frames}

Here, we consider examples of locally introspective and introspective theories based not just on $\Psh{\omega}$, but also on the relation between this and the discrete product of countably many copies of $\Set$. This is of particular note in connection with Kripke frames (see \TODOinline{cite later chapter}).
...
By $|P|_{< q}$ (where $q$ is any value in $Q$), we mean the discrete subset of $|P|$ comprising those values which, within $Q$, are less than $q$. Note that when $p \leq q$, there is an obvious forgetful functor from $\Psh{|P|_{< q}}$ to $\Psh{|P|_{< p}}$ given by projecting away unnecessary components. The composition of any number of such forgetful functors is another forgetful functor of the same form.
...
Note that $\Glob{C} = \Psh{|P|_{< \infty}} = \Psh{|P|}$. (Here, the addition of $\infty$ into $Q$ plays an important role when $P$ contains maximal elements. If we took $C$ to be merely a $P$-indexed category, then we would find that $\Glob{C}$ ignored any maximal elements in $P$. This is the only motivation for our introduction of $\infty$.)
...
There are obvious restriction maps from $\PshUnderQ{q}$ to $\PshUnderQ{p}$ for $p < q$ given by projection away of components. As a result, we have a $\Psh{Q}$-internal lexcategory $C'$ whose component at $q \in Q$ is given by $\PshUnderQ{q}$.

We may inductively choose our $\Set_q$ so that $\Ob(\PshUnderQ{q})$ and $\Mor(\PshUnderQ{q})$ are both objects of $\Set_q$ for each $q \in Q$. When we have done so, it follows that $C'$ is not merely internal to $\Psh{Q}$ but furthermore lives within $\PshUnderQInf$.

We observe that there is an obvious forgetful lexfunctor $\introS : \PshUnderQInf \to \Glob{C'} = \PshUnderQ{\infty}$, given by simply forgetting the aspect at $\infty$ and the restriction maps.
...
Finally, we define $\introN$ in the same way as above, with $\introN_X(q) : X(q) \to \Hom_{C'}(1, \introS(X))(q)$ given by the product of all the restriction maps out of $X(q)$.
