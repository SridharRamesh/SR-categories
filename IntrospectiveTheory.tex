...
The last two constructions are often fruitfully combined: Given a pre-introspective theory $\langle T, C, \introF \rangle$, we may first pass from $T$ to a sub-lexcategory $S$ of $T$ and then, after having done so, find that $\introF$ when restricted to $S$ factors through a sub-lexcategory $D$ of $C$. Thus, we obtain the following notion:

\begin{construction}\label{SubIntrospectionDefn}
When a pre-introspective theory $\langle S, D, G \rangle$ is obtained from a pre-introspective theory $\langle T, C, \introF \rangle$ by the combination of \magicref{SubTPreIntrosp} and \magicref{SubCPreIntrosp} as illustrated below, we say that $\langle S, D, G \rangle$ is a \defined{sub-introspection} of $\langle T, C, \introF \rangle$.

% https://q.uiver.app/?q=
\[\begin{tikzcd}
	{\op{S}} && {\op{T}} && \LexCat \\
	\\
	{\op{S}} &&&& \LexCat
	...
	\arrow[""{name=2, anchor=center, inner sep=0}, "{T'/-}", shift left=5, curve={height=-30pt}, from=1-1, to=1-5]
	\arrow[""{name=3, anchor=center, inner sep=0}, "{S/-}", shift left=5, from=3-1, to=3-5]
	\arrow[""{name=4, anchor=center, inner sep=0}, "D"{description}, from=3-1, to=3-5]
	\arrow[""{name=5, anchor=center, inner sep=0}, "{\pullAlong{i}C}"', shift right=5, from=3-1, to=3-5]
	\arrow["\introF"', shorten <=1pt, shorten >=1pt, Rightarrow, from=0, to=1]
	\arrow["i"', shorten <=4pt, Rightarrow, from=2, to=1-3]
	\arrow["j"', shorten <=3pt, shorten >=1pt, Rightarrow, from=4, to=5]
	\arrow["G"', shorten <=1pt, shorten >=3pt, Rightarrow, from=3, to=4]
\end{tikzcd}\]

In the above pair of diagrams, $i : S \to T$ is the inclusion exhibiting $S$ as a full sublexcategory of $T$ (thus, $i$ is a full and faithful lexfunctor). Similarly, $j : D \to \pullAlong{i} C$ is the inclusion exhibiting $D$ as an indexed full sublexcategory of $\pullAlong{i}C$ (thus, $j$ is an indexed lexfunctor, all of whose aspects are full and faithful). Both shown composite 2-cells from $S/-$ to $\pullAlong{i} C$ are taken to be identical; that is, the composite 2-cell of the top diagram is taken to factor through the inclusion $j$, thus yielding $G$.
\end{construction}
