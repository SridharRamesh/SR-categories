...
The first example in this section will eventually be used to relate introspective theories to the topos of trees as used in much work on guarded recursion. This may also be useful to build up intuition as we work towards the more complicated example in this section.

The final example in this section will eventually be particularly useful for relating introspective theories to the traditional study of \Goedel/-\Loeb/ modal logic via well-founded Kripke frames.

\paragraph{The locally introspective topos of trees and fully introspective subcategories of this}
Here, we consider an example of an introspective theory based on presheaves over the natural numbers (which comprise the so-called topos of trees). Analogous examples may be constructed for presheaves over arbitrary categories; see \TODOinline{cite later chapter} for a vast generalization. We introduce this particular example as it is perhaps the simplest nontrivial presheaf category to consider, and as this topos of trees has already been much considered in work on guarded recursion (a point we will return to in \TODOinline{cite later chapter}).
...
Also, as with any functor between presheaf categories given by composition in this manner, $\Prior$ has a right adjoint, given by right Kan extension. [The right adjoint of $\Prior$ may be called $\Later$; it can be described by $\Later(P)(0) = 1$ and $\Later(P)(n + 1) = P(n)$ for $n \in \omega$, with the obvious corresponding actions on restriction maps and on morphisms between presheaves. Note that we may pull the map $\prior : \Prior \to \id$ through the adjunction $\Prior \dashv \Later$ to obtain a corresponding map $\latermap : \id \to \Later$.]
...
[Pedantically, in this diagram, $\LexCat$ must be interpreted as suitably large relative to $\Psh{\omega}$ for the self-indexing of $\Psh{\omega}$ to be valued in $\LexCat$.]
...
Furthermore, the map $\Hom_C(1, \introS(-)) : \Psh{\omega} \to \Psh{\omega}$ can be seen to be the right adjoint to $\Prior$; thus, it is $\Later$. Finally, as for the $\introN$ corresponding to our $\introF$, this will be the map $\latermap : \id \to \Later$ given by pulling $\prior : \Prior \to \id$ through the adjunction $\Prior \dashv \Later$.]
\end{construction}
...
We do not have that $\Ob(C)$ is itself an object of $\Psh{\omega}$. Essentially, the obstruction is that $C(\yoneda(n)) = \Psh{\omega}/\Prior(\yoneda(n)) = \Psh{\omega}/\yoneda(\mathrm{max}(n - 1, 0))$ has a proper class of objects, but the presheaves in $\Psh{\omega}$ are set-valued.
...
One such choice, for example, is where we take each $\Set_n$ to comprise the sets of rank less than \TODO.

When we choose $\Set_n$ in this manner, we have that $C'$ is not only internal to $\Psh{\omega}$, but indeed lives within its full subcategory $\PshUnder$.

We now flesh $\langle \PshUnder, C'\rangle$ out into an introspective theory, by defining an appropriate $\introS$ and $\introN$. Much like before, $\Glob{C'}$ is readily identified with $\PshUnder$ and we take  $\introS$ to be this identification. As just as before, we find that under this identification, $\Hom_C(1, -)$ acts as $\Later$, so we may take $\introN$ to be $\latermap$. \TODOinline{Flesh this out more.}

This completes the description of $\langle \PshUnder, C' \rangle$ as an introspective theory  (relative to any suitable choice of the categories $\{\Set_n\}_{n \in \omega}$).
\end{construction}

\paragraph{A more archetypal example}

\TODOinline{Finish writing out the details on this example. Get rid of the "hopefully" words. Write out a naive version first, without size conditions, then show the obstruction, to motivate the size condition. Perhaps write out first a model like the topos of trees, where T and C are essentially the same, then do the more archetypal example after that.}
