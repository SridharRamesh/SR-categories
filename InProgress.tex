...
Now let $\Gamma[M^{-1}]_C : \Glob{C} \to IAU$ be the composition $f \circ \Gamma_C$. By whiskering along $f$, we get $f \introN : f \to f \circ \Gamma_C \circ \introS = f \circ \Gamma_C \circ g \circ f$.

By the properties of localization with respect to natural transformation, this corresponds to some unique natural transformation $H : \id_{IAU[M^{-1}]} \to f \circ \Gamma_C \circ g$, such that $f \introN = H f$.

Now, let's define $C'$ as $f[C]$. Let $\Gamma_C' : \Glob{C'} \to IAU[M^{-1}]$ be the corresponding global sections functor. Note that we have also $\InducedHomo{f}{S} : \Glob{C} \to \Glob{C'}$. Let $\introS' : IAU[M^{-1}]$ be given as the composition $\InducedHomo{F}{S} \circ g$. Finally, we need a natural transformation $\introN' : \id_{IAU[M^{-1}]} \to \Gamma_C' \circ \introS' = \Gamma_C' \circ \InducedHomo{f}{S} \circ g$. We obtain this $\introN'$ by taking $H$ as above and composing it with the whiskering along $g$ of the canonical natural transformation from $f \circ \Gamma_C$ to $\Gamma_C' \circ \InducedHomo{f}{S}$.
...
\subsection{GL lemma}
\begin{lemma}
Given $c, d \in C_2$, we have that $F_1(\Hom{C_2}(c, d)) = \Hom_{C_3}(F_1(c), F_1(d))$.
\end{lemma}

\subsection{TODO test}
Here's a comment: \sTODOinline{Hello}

This needs to be fleshed out: \sTODO

\sTODOinline{More stuff to be done.}
