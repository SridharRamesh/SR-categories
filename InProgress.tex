...
Then using the \quote{calculus of (right) fractions}, we have a tractable explicit construction of the category $T[M^{-1}]$ which freely makes each arrow in $M$ an isomorphism. That is, we have a functor $f : T \to T[M^{-1}]$ such that $f(m)$ is an isomorphism for each $m \in M$, and furthermore, for any other category $U$ and functor $g : T \to U$ such that $g(m)$ is an isomorphism for each $m \in M$, we have that $g$ factors uniquely through $f$ by another lexfunctor. In fact, this $T[M^{-1}]$ is a lexcategory and $f$ is a lexfunctor. When $g$ is a lexfunctor, its factorization through $f$ is by a lexfunctor as well \TODOinline{This last statement isn't explicitly in Borceux yet, but follows like so: It's easy enough to see the factoring functor preserves products, since the objects of $T[M^{-1}]$ are the same as those of $T$. Preservation of equalizers similarly follows from the fact that any two parallel maps in $T[M^{-1}]$ are given by an isomorphism in $M^{-1}$ followed by two parallel maps from $T$.}.
